...
MANAGED_BEGIN = "# >>> context-hub migrated MCP >>>"
MANAGED_END = "# <<< context-hub migrated MCP <<<"
...
def render_mcp_block(servers: List[McpServerSpec]) -> str:
    ...
    lines: List[str] = [MANAGED_BEGIN, "# Generated by scripts/migrate_context_hub_to_codex.py", ""]
...
def merge_mcp_into_codex_config(fragment_text: str, config_path: Path, dry_run: bool) -> None:
    ...
    if dry_run:
        return
    backup_path = config_path.with_suffix(config_path.suffix + ".bak.context-hub")
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Migrate Context Hub skills and MCP config into Codex."
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Where to write the generated MCP TOML fragment. Defaults to <repo-root>/out/context-hub-codex-mcp.toml.",
    )
    parser.add_argument(
        "--skill-prefix-mode",
...
def main() -> int:
    ...
    if args.mcp_fragment_output is None:
        fragment_output = repo_root / "out" / "context-hub-codex-mcp.toml"
    else:
